...
chunk = []
while isTruncated:
    response = bos_client.list_objects(bucket_name, prefix = 'picture', max_keys = max_keys, marker=marker)
    for obj in response.contents:
        if 'picture' not in obj.key or obj.key == 'picture/':
            continue
...
for i in xrange(len(data)):
    with open('../data/picture-' + str(i) + '.txt', 'w') as writer:
        for url in data[i]:
            try:
                writer.write('%s\n' % (url))
    ...
    bos_client.put_object_from_file(bucket=bucket_name,
            key = 'meta-info/picture-' + str(i) + '.txt', file_name = '../data/picture-' + str(i) + '.txt', storage_class=storage_class.STANDARD)
